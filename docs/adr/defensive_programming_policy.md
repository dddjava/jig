...
2. ミュータブルインスタンスに対する防御的コピーを行わない
   ...
   - コレクションなどの可変値を受け渡す場合、受け取り側は受領後に値が変更されないものとして扱います。渡し側は、受け渡した後にその値を変更してはなりません。
...
代わりに、共有後に変更しない契約を合意します。record はコンポーネントの可変性を自動的に排除しないため、渡し側がコレクションを変更しない責務を負います。受け取り側はこの契約を信頼し、防御的コピーや変更検知を行いません。
