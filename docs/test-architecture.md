...
- 上記とは別に CircleCI があり、main の push でビルドを、タグ push で Maven Central と Gradle Plugin Portal への公開を行う。CircleCI は廃止する方向だが、廃止までは実際の公開を担い続ける。廃止は別スケジュールで扱い、**この設計では CircleCI の設定を変更しない**。
...
互換性契約には二つの軸がある。JIG 自体を動かす環境（JDK 21、OS、Gradle、ブラウザ）と、JIG が読み取る対象（Java 8 以上でコンパイルされたクラスファイル）である。後者は README に明記した約束でありながら、現在は fixture が toolchain 21 でしか作られていないため未検証である。下限バージョンのサポートは、fixture をコンパイルできる範囲と一致させる（後述）。
...
- 解析対象バージョンの検証には、同一ソースを `--release 8` `--release 21` `--release 25` でコンパイルした三つの出力を作る。全 fixture を三重にはせず、この目的の fixture のみに適用する。
- TestKit は別プロセスで Gradle を起動するため、テンポラリディレクトリへ代表プロジェクトを展開する必要がある。展開も fixture API の責務に含める。各テストがビルドスクリプト文字列を個別に組み立てない。

`--release 8` は JDK 25 でも利用できるが、廃止予定として警告が出る。**最新 LTS で `--release 8` が指定できなくなった時点で、Java 8 の解析対象サポートを終了する**。事前コンパイル済みのクラスファイルを fixture に持ってサポートを延命しない。fixture がツールチェーンで再現できる範囲とサポート範囲を一致させ、検証できない約束を残さないためである。サポート終了時は fixture の対象バージョンを新しい下限へ入れ替える。
...
1. この設計をもとに `docs/adr/` の既存 ADR と同じ形式（状況／決定／トレードオフ／採用ガイドライン／結論）で ADR を書き起こして承認し、サポート対象の JDK、Gradle、ブラウザ、および解析対象の Java バージョンとその下限を切る条件を明文化する。現状 JDK は 21 単一であり、解析対象は Java 8 以上を約束しているという事実を出発点として記す。
2. テストタスク（`contractTest` `e2eTest` `qualityCheck`）と JaCoCo を導入する。PR CI は `build` を維持したまま `qualityCheck` を追加し、`npm run test:pr` の新設・`test:full` の拡張と、CLAUDE.md のテスト実行ポリシーの段階分けを行う。この時点では既存テストを移動しない。
3. `jig-test-fixtures` モジュールを追加し、代表プロジェクトのソースセット定義・`--release` 切り替え・展開・正規化・スナップショット更新手順を整える。最初の代表プロジェクトとして `minimal-java` と `bytecode-compat` を置き、解析対象バージョンの契約を PR で検証できる状態にする。
