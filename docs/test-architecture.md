...
互換性契約には二つの軸がある。JIG 自体を動かす環境（JDK 21、OS、Gradle、ブラウザ）と、JIG が読み取る対象（Java 8 以上でコンパイルされたクラスファイル）である。後者は README に明記した約束でありながら、現在は fixture が toolchain 21 でしか作られていないため未検証である。
...
`--release 8` は JDK 25 でも利用できるが、廃止予定として警告が出る。将来利用できなくなった場合は、当該バージョンのクラスファイルを事前コンパイル済みバイナリとして fixture に持つ方式へ切り替える。切り替え時期の判断材料として、コンパイル警告を無視せず記録する。
...
1. この設計をもとに `docs/adr/` の既存 ADR と同じ形式（状況／決定／トレードオフ／採用ガイドライン／結論）で ADR を書き起こして承認し、サポート対象の JDK、Gradle、ブラウザ、および解析対象の Java バージョンを明文化する。現状 JDK は 21 単一であり、解析対象は Java 8 以上を約束しているという事実を出発点として記す。
