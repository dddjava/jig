    ...
    public String buildJson(JigRepository jigRepository) {
        ...
        var libraries = diagram.libraries().stream()
                .map(library -> Json.object("id", library.id())
                        ...
                        .and("samplePackages", Json.array(library.samplePackages()))
                        .and("usingClasses", Json.array(library.usingClasses())))
                .toList();
        ...
        return Json.object("internalPackages", Json.array(diagram.internalPackageFqns()))
