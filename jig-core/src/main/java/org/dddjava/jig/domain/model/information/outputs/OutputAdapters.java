...
public record OutputAdapters(Collection<OutputAdapter> values) {
    ...
    public static OutputAdapters from(JigTypes jigTypes, PersistenceAccessorsRepository persistenceAccessorsRepository) {
        return jigTypes.orderedStream()
                .filter(jigType -> jigType.typeCategory() == TypeCategory.OutputAdapter)
                .map(jigType -> OutputAdapter.from(jigType, jigTypes, persistenceAccessorsRepository))
