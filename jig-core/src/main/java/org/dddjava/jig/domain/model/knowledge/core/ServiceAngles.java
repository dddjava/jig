...
public record ServiceAngles(Collection<Entry> entries) {
    ...
    public static ServiceAngles from(ServiceMethods serviceMethods, InputAdapters inputAdapters, DatasourceMethods datasourceMethods) {
        ...
        return new ServiceAngles(serviceMethods
                .streamAndMap((jigType, serviceMethodList) -> {
                    var serviceAngleList = serviceMethodList.stream()
                            .map(serviceMethod -> ServiceAngle.from(serviceMethod, serviceMethods, inputAdapters, datasourceMethods))
                            .toList();
