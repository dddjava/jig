const OutboundApp = (() => {
    ...
    /**
     * 2. モデル構築 (Model Builder)
     */
    const ModelBuilder = {
        build(data) {
            const grouped = this.groupOperationsByOutboundPort(data);
        ...
        groupOperationsByOutboundPort(data) {
        ...
        groupOperationsByPersistenceTarget(operations) {
            const map = new Map();
        ...
        groupOperationsByExternalType(operations) {
            const map = new Map();
            operations.forEach(operation => {
        ...
        isCrudVisible(operationType, visibility = state.visibility || DEFAULT_VISIBILITY) {
    ...
    const Renderer = {
        render(state) {
            this.updateTabs(state.activeTab);
            this.renderAllPanels(state);
        },
        ...
        renderAllPanels(state) {
            ...
            this.renderPersistenceList(persistenceGrouped, visibility);
            this.renderExternalList(externalGrouped, visibility);
            this.renderOutboundList(grouped, visibility);
            this.renderCrudTable(grouped);
        },

        renderOutboundList(grouped, visibility = state.visibility || DEFAULT_VISIBILITY) {
            ...
            grouped.forEach(group => {
                const portMermaidCode = this.generatePortMermaidCode(group, visibility);
                ...
                Jig.mermaid.diagram.createAndRegister(cardChildren, (container) => {
                    ...
                    if (generator(currentVisibility.direction)) {
                        this.renderMermaid(generator, container, {direction: currentVisibility.direction});
                    }
                ...
                group.operations.forEach(operation => {
                    ...
                    Jig.mermaid.diagram.register(mermaidContainer, () => {
                        const currentVisibility = App.readVisibility();
                        const generator = (dir) => this.generateOperationMermaidCode(operationWithPort, {
                            ...currentVisibility,
                            direction: dir
                        });
                        if (generator(currentVisibility.direction)) {
                            this.renderMermaid(generator, mermaidContainer, {direction: currentVisibility.direction});
                        }
                    });
            ...
            if (grouped.length === 0) this.renderNoData(container);
        },

        renderPersistenceList(grouped, visibility = state.visibility || DEFAULT_VISIBILITY) {
            ...
            grouped.forEach(group => {
                const persistenceMermaidCode = this.generatePersistenceMermaidCode(group, visibility);
                ...
                Jig.mermaid.diagram.register(persistenceMermaidContainer, () => {
                    const currentVisibility = App.readVisibility();
                    const generator = (dir) => this.generatePersistenceMermaidCode(group, {...currentVisibility, direction: dir});
                    if (generator(currentVisibility.direction)) {
                        this.renderMermaid(generator, persistenceMermaidContainer, {direction: currentVisibility.direction});
                    }
                });
        ...
        renderExternalList(grouped, visibility = state.visibility || DEFAULT_VISIBILITY) {
            ...
            grouped.forEach(group => {
                const externalMermaidCode = this.generateExternalTypeMermaidCode(group, visibility);
                ...
                Jig.mermaid.diagram.register(externalMermaidContainer, () => {
                    const currentVisibility = App.readVisibility();
                    const generator = (dir) => this.generateExternalTypeMermaidCode(group, {...currentVisibility, direction: dir});
                    if (generator(currentVisibility.direction)) {
                        this.renderMermaid(generator, externalMermaidContainer, {direction: currentVisibility.direction});
                    }
                });
        ...
        renderCrudTable(grouped) {
            ...
            const allPersistenceTargets = ModelBuilder.collectAllTargets(grouped);
                            ...
                            group.operations.forEach(operation => {
                                operation.persistenceAccessors.forEach(op => {
                                    if (persistenceTarget in op.targetOperationTypes) {
                                        const crud = ModelBuilder.toCrudChar(op.targetOperationTypes[persistenceTarget]);
        ...
        renderMermaid(diagramFn, container, options = {}) {
        ...
        generatePortMermaidCode(group, visibility = state.visibility || DEFAULT_VISIBILITY) {
            ...
            group.operations.forEach((operation) => {
                ...
                let lastNodeId = this.addPortNode(builder, contexts.portSubgraphs, portFqn, portLabel, portOpFqn, portOpName, visibility);
                lastNodeId = this.addAdapterNode(builder, lastNodeId, adapterFqn, adapterLabel, executionFqn, executionName, visibility, contexts.adapterSubgraphs);

                operation.persistenceAccessors.forEach(op => {
                    const currentNode = this.addAccessorNode(builder, lastNodeId, op, visibility, contexts.accessorSubgraphs, contexts.accessorNodes);
                    if (visibility.target) {
                        this.addPersistenceTargetEdges(builder, currentNode, op, contexts.persistenceTargetNodes, visibility);
                    }
                });

                operation.externalAccessors.forEach(accessor => {
                    this.addExternalAccessorNode(builder, lastNodeId, accessor, visibility, contexts.extAccessorNodes, contexts.extAccessorSubgraphs, contexts.extTypeNodes);
                });
            });
        ...
        generateOperationMermaidCode(operation, visibility = state.visibility || DEFAULT_VISIBILITY) {
            return this.generatePortMermaidCode({outboundPort: operation.outboundPort, operations: [operation]}, visibility);
        },

        generatePersistenceMermaidCode(group, visibility = state.visibility || DEFAULT_VISIBILITY) {
            const builder = new Jig.mermaid.Builder();
            builder.applyThemeClassDefs();
            ...
            group.operations.forEach((operation) => {
                ...
                operation.persistenceAccessors
                    ...
                    .filter(op => ModelBuilder.isCrudVisible(op.targetOperationTypes[persistenceTarget], visibility))
                    .forEach(op => {
                        ...
                        currentNode = this.addAccessorNode(builder, currentNode, op, visibility, contexts.accessorSubgraphs, contexts.accessorNodes);

                        if (visibility.target) {
                            this.addPersistenceTargetEdges(builder, currentNode, {
        ...
        generateExternalTypeMermaidCode(group, visibility = state.visibility || DEFAULT_VISIBILITY) {
            const builder = new Jig.mermaid.Builder();
            builder.applyThemeClassDefs();
        ...
        addPortNode(builder, portSubgraphs, portFqn, portLabel, portOpFqn, portOpName, visibility) {
        ...
        addAdapterNode(builder, sourceNodeId, adapterFqn, adapterLabel, executionFqn, executionName, visibility, adapterSubgraphs) {
        ...
        addAccessorNode(builder, sourceNodeId, op, visibility, accessorSubgraphs, accessorNodes) {
        ...
        addPersistenceTargetEdges(builder, sourceNodeId, op, persistenceTargetNodes, visibility) {
            Object.entries(op.targetOperationTypes).forEach(([persistenceTarget, operationType]) => {
                if (!ModelBuilder.isCrudVisible(operationType, visibility)) return;
        ...
        addExternalAccessorNode(builder, sourceNodeId, accessor, visibility, extAccessorNodes, extAccessorSubgraphs, extTypeNodes) {
    ...
    const App = {
        init() {
            ...
            state.data = DataLoader.load();

            const model = ModelBuilder.build(state.data);
        ...
        setState(newState) {
            ...
            if ('visibility' in newState) {
                Renderer.renderAllPanels(state);
                Jig.mermaid.diagram.rerenderVisible();
            }
            if ('activeTab' in newState) {
                Renderer.updateTabs(state.activeTab);
            }
        },
        ...
        bindEvents() {
            ...
            document.querySelectorAll('input[name^="show-"]').forEach(input => {
                input.addEventListener('change', () => {
                    updateChildDisabled();
                    this.setState({visibility: this.readVisibility()});
                });
            });
            ...
            document.querySelectorAll('.outbound-tab-list .tab-button').forEach(button => {
                button.addEventListener('click', () => {
                    this.setState({activeTab: button.getAttribute('data-tab')});
                });
            });
        },
    ...
    return {
        ...
        state,
        // テスト互換性のための公開API
        getOutboundData: DataLoader.load,
        ...
        renderExternalList: Renderer.renderExternalList.bind(Renderer),
        renderCrudTable: Renderer.renderCrudTable.bind(Renderer),
        toCrudChar: ModelBuilder.toCrudChar.bind(ModelBuilder),
        renderNoData: Renderer.renderNoData.bind(Renderer),
        generateOperationMermaidCode: Renderer.generateOperationMermaidCode.bind(Renderer),
        generatePortMermaidCode: Renderer.generatePortMermaidCode.bind(Renderer),
        generatePersistenceMermaidCode: Renderer.generatePersistenceMermaidCode.bind(Renderer),
        generateExternalTypeMermaidCode: Renderer.generateExternalTypeMermaidCode.bind(Renderer),
        addExternalAccessorNode: Renderer.addExternalAccessorNode.bind(Renderer),
    };
})();
