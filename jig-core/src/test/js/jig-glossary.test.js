...
test.describe("fqnToId", () => {
    test("プレフィックスを付けてIDを生成する", () => {
        const id = sut.fqnToId("port", "com.example.MyPort");
        assert.match(id, /^port_MyPort_exa_[a-z0-9]+$/);
    });

    test("異なるfqnなら異なるIDを生成する", () => {
        const id1 = sut.fqnToId("persistence", "my_table");
        const id2 = sut.fqnToId("persistence", "another_table");
        assert.notEqual(id1, id2);
    });

    test("同じfqnなら同じIDを生成する（一意性）", () => {
        const id1 = sut.fqnToId("op", "com.example.Port#save(java.lang.String)");
        const id2 = sut.fqnToId("op", "com.example.Port#save(java.lang.String)");
        assert.equal(id1, id2);
    });

    test("マルチバイト文字でも正しくハッシュ化される", () => {
        const id1 = sut.fqnToId("persistence", "テーブル1");
        const id2 = sut.fqnToId("persistence", "テーブル2");
        assert.notEqual(id1, id2);
